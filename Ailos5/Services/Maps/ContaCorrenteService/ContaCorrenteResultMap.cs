...
{
    public class ContaCorrenteResultMap : IMapperSpecific<ContaCorrente, Entitie.ContaCorrente>
    {
        public async Task<ContaCorrente> MapperAsync(Entitie.ContaCorrente? item)
        {
            ...
            return new ContaCorrente(item.Id, item.Guid, item.NumeroDaConta, item.NomeDoCliente, item.Ativo);
        }
